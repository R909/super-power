...
export default function CTA() {
  ...
  useEffect(() => {
    ...
    // --- PHASE 1: ELEVATED SCROLL INTRANCE ---
    ...
    // --- PHASE 2: MAGNETIC BUTTON HOVER MECHANICS ---
    if (button && !("ontouchstart" in window)) {
      const onMouseMove = (e: MouseEvent) => {
        const rect = button.getBoundingClientRect();
        // Calculate relative offset from button center point
        const x = e.clientX - (rect.left + rect.width / 2);
        const y = e.clientY - (rect.top + rect.height / 2);

        // Core magnetic attraction pulling the physics layer toward cursor
        gsap.to(button, {
          x: x * 0.35,
          y: y * 0.35,
          duration: 0.3,
      ...
      const onMouseLeave = () => {
        // Snap back cleanly when focus breaks
  ...
  return (
    ...
    >
      ...
      {/* ☀️ Massive Golden Eclipse Background Emission Ring */}
      ...
      <div className="fade-up-cta opacity-0 max-w-3xl flex flex-col items-center relative z-10 [transform-style:preserve-3d]">
        ...
        {/* 🧲 High-End Premium Magnetic Trigger CTA Action Hub */}
        <div className="relative p-4 [transform-style:preserve-3d]">
          {/* Subtle glow layer behind button path */}
